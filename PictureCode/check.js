...
//function to highlight the points
function highlight(point){

    var square = defineRectangle(point.x, point.y);
...
function mousedown_event(e) {
	...
	if(clickedPoints.length <= 5) {
        if(clickedPoints.length) highlight(clickedPoints[ clickedPoints.length - 1 ]);
	}
}
...
function animate() {
	...
	if(clickedPoints.length == 5 && once) {
        once = false;
        console.log("5 points selected!");

        text = "";
        for(i = 0; i < 5; i ++)
            text += clickedPoints[i].x + " " + clickedPoints[i].y + ";";
